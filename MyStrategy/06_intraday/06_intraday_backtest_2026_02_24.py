...
def backtest(para_comb):
    ...
    pct_chg_sign    = para_comb['pct_chg_sign']
    pct_chg_thres   = para_comb['pct_chg_thres']
    ...
    if pct_chg_sign == 'positive':
        df['trade_logic'] = (df['pct'] > pct_chg_thres * 0.01)
    if pct_chg_sign == 'negative':
        df['trade_logic'] = (df['pct'] < -1 * pct_chg_thres * 0.01)
    ...
    for i, row in df.iterrows():
        now_d = pd.to_datetime(i).date()
        now_t = pd.to_datetime(i).time()
        now_open = row['open']
        now_high = row['high']
        now_low = row['low']
        now_close = row['close']
        ...
        t_diff_hr = (now_d - open_t).total_seconds() / 3600
        if (freq == '1D'):
            num_of_res = 24
        else:
            num_of_res = int(freq[:-1])
...
if __name__ == '__main__':
    ...
    para_dict = {
        ...
        'pct_chg_sign': ['positive', 'negative'],
        'pct_chg_thres': [0.2, 0.5, 0.8, 1, 1.2, 1.5, 1.8, 2, 2.5, 3, 3.5, 4],
    }
